...
Per-file conventions for domain artifacts live in `.Codex/rules/domains/` (one rule per file type). They are path-scoped, so Codex loads the matching rule automatically when editing e.g. a `service.py` or `repository.py`.
