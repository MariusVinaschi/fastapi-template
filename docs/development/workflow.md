...
types, complexipy (12 per function) and tests with coverage. Coverage has no
new blocking threshold. Check results are local working artifacts.
...
Reuse successful check evidence only while its code fingerprint is current;
otherwise rerun the affected checks. If tooling cannot provide independent
