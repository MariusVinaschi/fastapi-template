"""
User service - Framework agnostic business logic layer.
...
class UserService(
    ...
    def _check_general_permissions(self, action: str) -> bool:
        """Check if user has permission to perform action on users in general"""
    ...
    def _check_instance_permissions(self, action: str, instance: User) -> bool:
        """Check if user has permission to perform action on specific user instance"""
    ...
    async def get_by_email(
        ...
        """Get user by email with authorization checks"""
    ...
    async def get_by_clerk_id(self, clerk_id: str) -> User:
        """Get user by Clerk ID with authorization checks"""
        if not self._is_system_operation():
            self._check_general_permissions("read")
...
class ClerkUserService(UserService):
    """
    Extended user service for handling Clerk webhook events.
    This service handles user synchronization with Clerk.
    """
    ...
    def _check_clerk_id(data: dict) -> str:
        """Validate and extract Clerk ID from payload"""
        if not data.get("id"):
...
class APIKeyService(
    ...
    @classmethod
    def for_user(cls, session: AsyncSession, authorization_context: AuthorizationContext):
        """Factory method for user operations"""
        return cls(session, authorization_context)

    @classmethod
    def for_system(cls, session: AsyncSession):
        """Factory method for system operations"""
        return cls(session)

    def _prepare_create_data(self, data: APIKeyCreate) -> dict:
        """Prepare API key data for creation"""
    ...
    async def generate_api_key(self, user: User) -> APIKeyGenerated:
        """Generate a new API key for user, replacing any existing one"""
        try:
        ...
        api_key = f"template_{secrets.token_urlsafe(32)}"
