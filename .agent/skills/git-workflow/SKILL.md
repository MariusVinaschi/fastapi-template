---
name: git-workflow
description: GitHub Flow patterns — a single, trunk-protected branching strategy with feature branches and PRs — plus commit conventions, merge vs rebase, conflict resolution, and collaborative development best practices. Use when setting up Git workflow for a project, writing commit conventions, deciding merge versus rebase, or resolving conflicts.
---

# Git Workflow Patterns (GitHub Flow)

Best practices for Git version control and collaborative development,
built around GitHub Flow as the project's single branching strategy.
...
- Keep feature branches short-lived — the longer a branch lives, the more
  it drifts from `main` and the more conflict risk it accumulates
...
Under GitHub Flow, releases are typically tagged directly off `main` after
merge, since `main` is always deployable — there's no separate `develop`
or `release/*` branch to manage.
...
```bash
# Create annotated tag directly on main
git checkout main
