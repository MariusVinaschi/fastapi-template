...
async def test_find_by_email(db_session):
    ...
    user = await UserService.for_user(db_session, UserAuthorizationAdapter(user)).get_by_email(user.email)
    assert user == user
...
async def test_find_by_email_not_found(db_session):
    ...
    with pytest.raises(UserNotFoundException):
        await UserService.for_user(db_session, UserAuthorizationAdapter(user)).get_by_email("nonexistent@example.com")
