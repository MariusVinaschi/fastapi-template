...
async def test_update_user(db_session):
    ...
    updated_user = await ClerkWebhookAdapter.for_system(db_session).update_user(data)
...
async def test_update_user_email_already_exists(db_session):
    clerk_id = "clerk_id_123"
    user = await UserFactory.create_async(session=db_session, clerk_id=clerk_id, email="old@example.com")
    data = {
        "id": clerk_id,
        "primary_email_address_id": "123",
        "email_addresses": [
...
async def test_update_user_not_found(db_session):
    ...
    with pytest.raises(UserNotFoundException):
        await ClerkWebhookAdapter.for_system(db_session).update_user(data)
