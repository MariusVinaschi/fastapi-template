...
def test_generated_values_override_the_committed_configuration(tmp_path, monkeypatch):
    # The ambient test environment would otherwise win, as process variables are applied last.
    for name in ("APP_DB_NAME", "APP_DB_USER"):
        monkeypatch.delenv(name, raising=False)
    (tmp_path / ".env").write_text("APP_DB_NAME=committed\nAPP_DB_USER=shared\n")
    write_env(tmp_path / ENV_FILE, {"WORKTREE_ROOT": str(tmp_path.resolve()), "APP_DB_NAME": "generated"})
    env = runtime_environment(tmp_path)
    assert env["APP_DB_NAME"] == "generated"
